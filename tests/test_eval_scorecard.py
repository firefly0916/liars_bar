...
class EvalScorecardTest(unittest.TestCase):
    ...
    def test_build_eval_scorecard_script_ignores_non_directory_inputs(self) -> None:
        script_path = Path("scripts/build_eval_scorecard.py").resolve()
        spec = importlib.util.spec_from_file_location("build_eval_scorecard_script", script_path)
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            experiment_root = root / "step-000200"
            task_m_dir = experiment_root / "task_m"
            task_1_1_dir = experiment_root / "task_1_1"
                ...
                json.dumps(
                    {
                        ...
                        "game_summaries": [{"winner": "p1", "turns_played": 1}],
                    },
                    ensure_ascii=False,
                ),
                encoding="utf-8",
            ...
            (task_1_1_dir / "summary.json").write_text(
                json.dumps(
                    {
                        "audited_turn_count": 1,
                        "negative_phi_turn_count": 0,
                        "conflict_count": 0,
                        "avg_ev_gap": 0.0,
                        "max_ev_gap": 0.0,
                        "high_ev_gap_turn_count": 0,
            ...
            (games_dir / "game-01.jsonl").write_text("", encoding="utf-8")
            stray_log = root / "master.log"
            stray_log.write_text("formal eval log\n", encoding="utf-8")
            argv_backup = sys.argv[:]
            try:
                sys.argv = [
                    "build_eval_scorecard.py",
                    str(experiment_root),
                    str(stray_log),
                    "--format",
                    "json",
    ...
    def test_build_eval_scorecard_script_fails_when_no_directory_inputs_exist(self) -> None:
        script_path = Path("scripts/build_eval_scorecard.py").resolve()
        spec = importlib.util.spec_from_file_location("build_eval_scorecard_script", script_path)
        self.assertIsNotNone(spec)
        self.assertIsNotNone(spec.loader)
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            stray_log = Path(temp_dir) / "master.log"
            stray_log.write_text("formal eval log\n", encoding="utf-8")
            ...
            try:
                ...
                with self.assertRaisesRegex(ValueError, "No valid experiment directories"):
                    module.main()
            finally:
                sys.argv = argv_backup
