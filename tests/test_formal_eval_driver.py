...
class FormalEvalDriverTest(unittest.TestCase):
    ...
    def test_load_checkpoint_tags_can_limit_selection_count(self) -> None:
        module = importlib.import_module("liars_game_engine.analysis.formal_eval_driver")
        ...
        self.assertEqual(tags, ["step-000200", "final", "step-000170"])
    ...
    def test_run_selected_formal_eval_passes_local_model_path_env_to_task_m(self) -> None:
        script_path = Path(__file__).resolve().parent.parent / "scripts" / "run_selected_formal_eval.py"
        spec = importlib.util.spec_from_file_location("run_selected_formal_eval_script", script_path)
        self.assertIsNotNone(spec)
        self.assertIsNotNone(spec.loader)
        ...
        self.assertEqual(exit_code, 0)
        self.assertGreaterEqual(len(captured_envs), 1)
        self.assertEqual(
            captured_envs[0]["LOCAL_LLM_MODEL_PATH"],
            "/root/autodl-tmp/models/huggingface/Qwen/Qwen2.5-7B-Instruct",
        )

    def test_run_selected_formal_eval_uses_selection_json_top_k_when_tags_not_passed(self) -> None:
        script_path = Path(__file__).resolve().parent.parent / "scripts" / "run_selected_formal_eval.py"
        spec = importlib.util.spec_from_file_location("run_selected_formal_eval_script", script_path)
        self.assertIsNotNone(spec)
        self.assertIsNotNone(spec.loader)
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            for tag in ("step-000200", "final", "step-000170", "step-000140"):
                (root / "train" / "checkpoints" / tag).mkdir(parents=True, exist_ok=True)
            selection_path = root / "shortlist.json"
            selection_path.write_text(
                json.dumps(
                    {
                        "selected": [
