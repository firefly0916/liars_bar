...
class LlmDrillTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_run_llm_drill_writes_game_logs_and_summary(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            progress_lines = progress_log.read_text(encoding="utf-8").splitlines()
            self.assertGreaterEqual(len(progress_lines), 3)
            self.assertIn("completed_turns=0", progress_lines[0])
            self.assertIn("total_turn_budget=80", progress_lines[0])
            self.assertIn("progress_bar=[--------------------]", progress_lines[0])
            self.assertTrue(any("completed_turns=" in line and "progress_bar=[" in line for line in progress_lines[1:]))
            self.assertIn("run_complete=true", progress_lines[-1])
