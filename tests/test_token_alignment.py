...
class TokenAlignmentTest(unittest.TestCase):
    def test_build_assistant_response_text_uses_action_first_protocol(self) -> None:
        module = importlib.import_module("liars_game_engine.analysis.token_alignment")
        sample = {
            "thought": "当前手里有一张真A，先小试探。",
        ...
        self.assertEqual(
            assistant_text,
            '{"Reasoning":"当前手里有一张真A，先小试探。","Action":{"type":"play_claim","play_count":2,"true_card_count":1}}',
        )

    def test_build_assistant_response_text_uses_null_counts_for_challenge(self) -> None:
        module = importlib.import_module("liars_game_engine.analysis.token_alignment")
        sample = {
            "thought": "我怀疑对方在撒谎。",
        ...
        self.assertEqual(
            assistant_text,
            '{"Reasoning":"我怀疑对方在撒谎。","Action":{"type":"challenge","play_count":null,"true_card_count":null}}',
        )
    ...
    def test_recalibrate_alignment_sample_reuses_holographic_messages(self) -> None:
        module = importlib.import_module("liars_game_engine.analysis.token_alignment")
        tokenizer = _FakeQwenTokenizer()
        sample = {
            "thought": "风险很高，但我想试探一下。",
            ...
            "alignment_metadata": {"assistant_text": "stale"},
            "token_weight_mask": [999.0],
        }

        recalibrated = module.recalibrate_alignment_sample(
            sample=sample,
            tokenizer=tokenizer,
            weight_distribution_strategy="equal",
        )

        self.assertIn("alignment_metadata", recalibrated)
        self.assertIn("token_weight_mask", recalibrated)
        self.assertEqual(recalibrated["messages"], sample["messages"])
        self.assertNotEqual(recalibrated["alignment_metadata"]["assistant_text"], "stale")
        self.assertNotEqual(recalibrated["token_weight_mask"], [999.0])
        self.assertGreater(recalibrated["alignment_metadata"]["assistant_rendered_span"]["start"], 0)
