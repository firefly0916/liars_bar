...
from liars_game_engine.agents.llm_agent import LlmAgent
...
class LlmAgentTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_action_only_agent_repairs_challenge_alias(self) -> None:
        agent = LlmAgent(
            player_id="p1",
            model="unit-test-model",
            prompt_profile="action_only",
            temperature=0.0,
            api_key="LOCAL",
            base_url="local://hf",
        )
        ...
        raw_output = '{"Action":{"type":"cast_doubt","play_count":null,"true_card_count":null}}'

        with patch.object(agent, "_request_raw_output", AsyncMock(return_value=raw_output)):
            decision = await agent.act(observation)

        self.assertEqual(decision.action.type, "challenge")
        self.assertIsNone(decision.parse_error)
        self.assertIn("action_only_protocol_repair", str(decision.resolution_reason))

    async def test_action_only_agent_repairs_missing_type_with_counts(self) -> None:
        agent = LlmAgent(
            player_id="p1",
            model="unit-test-model",
            prompt_profile="action_only",
        ...
        raw_output = '{"Action":{"play_count":1,"true_card_count":1}}'
    ...
    def test_build_openai_messages_supports_action_only_contract(self) -> None:
        profile = {
            **load_prompt_profile("baseline"),
            "instruction": (
                "Return JSON only with key Action. "
                "Do not output Reasoning, analysis, markdown, or any extra text. "
                "Action.type must be one legal action type from the current Status Report."
            ),
            "response_format": "action_only",
        }
        observation = {
            "player_id": "p1",
            "phase": "response_window",
        ...
        self.assertIn('"Action"', user_prompt)
        self.assertIn('"type": "play_claim|challenge"', user_prompt)
        self.assertIn("Do not output Reasoning", user_prompt)
        self.assertNotIn('"Reasoning": "string"', user_prompt)
