...
class OrchestratorLoggingTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_orchestrator_uses_legal_fallback_when_configured_fallback_is_illegal(self) -> None:
        settings = self._make_settings()

        class _IllegalOpeningAgent(BaseAgent):
            async def act(self, observation: dict[str, object]) -> AgentDecision:
                return AgentDecision(
                    thought="invalid opening fallback candidate",
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            orchestrator = GameOrchestrator(
                env=env,
                agents={
                    "p1": _IllegalOpeningAgent("p1", "stub", "baseline", 0.0),
                    "p2": _IllegalOpeningAgent("p2", "stub", "baseline", 0.0),
                },
                logger=logger,
                fallback_action=settings.runtime.fallback_action,
                max_turns=1,
            ...
            first_record = json.loads(logger.log_file.read_text(encoding="utf-8").splitlines()[0])
            self.assertTrue(first_record["fallback_used"])
            self.assertIn("challenge is only allowed", first_record["fallback_reason"])
            self.assertTrue(first_record["step_result"]["success"])
            self.assertIsNone(first_record["step_result"]["error_code"])
            self.assertTrue(
                any("played 1 face-down card" in event for event in first_record["step_result"]["events"])
            )
