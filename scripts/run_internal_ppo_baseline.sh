...
RUN_ROOT="${RUN_ROOT:-/root/autodl-tmp/experiments/$(date +%Y%m%d)-ppo-baseline}"
...
PPO_TRAIN_SCRIPT="${PPO_TRAIN_SCRIPT:-${TRAIN_REPO}/scripts/train_ppo_baseline.py}"
PPO_ADAPTER_PATH="${PPO_ADAPTER_PATH:-${TRAIN_ROOT}/checkpoints/final}"

TASKM_CONFIG="${TASKM_CONFIG:-${EVAL_REPO}/config/experiment.yaml}"
AUDIT_CONFIG="${AUDIT_CONFIG:-${AUDIT_REPO}/config/experiment.yaml}"

PPO_STEPS="${PPO_STEPS:-200}"
...
if [[ "${ALLOW_EXISTING_RUN_ROOT:-0}" != "1" ]]; then
  if [[ -e "${TRAIN_ROOT}/train_summary.json" || -e "${AUDIT_OUT}/summary.json" || -e "${SCORECARD_OUT}" ]]; then
    echo "ERROR: RUN_ROOT already contains PPO baseline outputs: ${RUN_ROOT}" >&2
    echo "Use a new RUN_ROOT, or set ALLOW_EXISTING_RUN_ROOT=1 only if you know the directory is clean enough to reuse." >&2
    exit 2
  fi
fi
...
env \
  ...
  python "${PPO_TRAIN_SCRIPT}" \
    ...
    --kl-coef "${PPO_KL_COEF}" \
    --clip-range "${PPO_CLIP_RANGE}" \
    --random-seed "${PPO_RANDOM_SEED}" \
    --save-final-adapter \
    "${PPO_EXTRA_ARGS_ARRAY[@]}" \
    > "${TRAIN_ROOT}/stdout.log" 2>&1

test -d "${PPO_ADAPTER_PATH}"

echo "===== PPO FORMAL TASK M $(date) ====="
cd "${EVAL_REPO}"
...
LOCAL_LLM_ADAPTER_PATH="${PPO_ADAPTER_PATH}" \
...
echo "===== DONE INTERNAL PPO BASELINE $(date) ====="
echo "Train log: ${TRAIN_ROOT}/stdout.log"
echo "PPO adapter: ${PPO_ADAPTER_PATH}"
