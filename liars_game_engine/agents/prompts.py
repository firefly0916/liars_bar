...
DEFAULT_PROFILE = {
    ...
    "response_format": "reasoning_action",
}
...
def _format_decision_request(legal_action_types: list[str], *, response_format: str) -> str:
    action_union = "|".join(legal_action_types)
    ...
    if response_format == "action_only":
        if "play_claim" in legal_action_types:
            action_schema = (
                '  "Action": {\n'
                f'    "type": "{action_union}",\n'
                '    "play_count": 1,\n'
            ...
            action_schema = (
                ...
                f'    "type": "{action_union}",\n'
                '    "play_count": null,\n'
                '    "true_card_count": null\n'
                "  }\n"
            )
        examples = []
        if "play_claim" in legal_action_types:
            examples.append(
                'Valid play_claim example: {"Action":{"type":"play_claim","play_count":1,"true_card_count":0}}'
            )
        if "challenge" in legal_action_types:
            examples.append(
                'Valid challenge example: {"Action":{"type":"challenge","play_count":null,"true_card_count":null}}'
            )
        if "pass" in legal_action_types:
            examples.append(
                'Valid pass example: {"Action":{"type":"pass","play_count":null,"true_card_count":null}}'
            )
        return (
            "Decision Request\n"
            "Choose one legal action. No hidden info.\n"
            f"- Valid Action.type values now: {action_union}.\n"
            "- Use ASCII English JSON only.\n"
            "- Use the exact Action.type string only; do not translate it and do not invent synonyms such as cast_doubt or cast_outrage.\n"
            "- If Action.type is play_claim, play_count MUST be an integer in the legal range and true_card_count MUST be an integer from 0 to play_count. Never use null for play_claim counts.\n"
            "- If Action.type is challenge or pass, play_count and true_card_count MUST be null.\n"
    ...
    return (
        ...
        "{\n"
        '  "Reasoning": "string",\n'
        f"{reasoning_action_schema}"
        "}"
    )


def format_observation_for_llm(observation: dict[str, object], *, response_format: str = "reasoning_action") -> str:
    ...
    normalized_response_format = str(response_format or "reasoning_action")
    ...
    return (
        ...
        f"{_format_decision_request(legal_action_types, response_format=normalized_response_format)}"
    )
...
def build_openai_messages(profile: dict[str, object], observation: dict[str, object]) -> list[dict[str, str]]:
    ...
    response_format = str(profile.get("response_format", "reasoning_action") or "reasoning_action")
    return [
        {
            "role": "system",
        ...
        },
        {
            ...
            "content": (
                ...
                f"{format_observation_for_llm(observation, response_format=response_format)}"
            ),
        },
    ]
