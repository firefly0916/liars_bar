...
class ProxyValuePredictor:
    def __init__(
        ...
        normalized_state_dict = self._normalize_proxy_state_dict(state_dict)
    ...
    def _normalize_proxy_state_dict(state_dict: dict[str, object]) -> dict[str, object]:
        if any(str(key).startswith("backbone.") or str(key).startswith("head.") for key in state_dict):
            return dict(state_dict)

        legacy_to_current = {
            "network.0.weight": "backbone.0.weight",
            "network.0.bias": "backbone.0.bias",
            "network.2.weight": "backbone.2.weight",
            "network.2.bias": "backbone.2.bias",
            "network.4.weight": "head.weight",
            "network.4.bias": "head.bias",
        }
