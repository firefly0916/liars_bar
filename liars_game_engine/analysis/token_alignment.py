...
def build_action_only_assistant_response_text(sample: dict[str, object]) -> str:
    payload = {
        "Action": _derive_action_target(sample),
    }
    return json.dumps(payload, ensure_ascii=False, separators=(",", ":"))
...
def _resolve_explicit_assistant_text(sample: dict[str, object]) -> str | None:
    assistant_target = sample.get("assistant_target")
    if isinstance(assistant_target, str) and assistant_target.strip():
        return assistant_target
    if isinstance(assistant_target, dict):
        return json.dumps(assistant_target, ensure_ascii=False, separators=(",", ":"))
    return None
...
def build_alignment_metadata(
    ...
    if reasoning_text and reasoning_text in resolved_assistant_text:
        reasoning_assistant_start, reasoning_assistant_end = _find_reasoning_span(
            resolved_assistant_text,
            reasoning_text,
        )
    else:
        reasoning_assistant_start, reasoning_assistant_end = 0, 0
    ...
    if not isinstance(strategic_tokens, list):
        strategic_tokens = []
    if reasoning_assistant_end <= reasoning_assistant_start:
        strategic_tokens = []
...
def recalibrate_alignment_sample(
    sample: dict[str, object],
    tokenizer: object,
    weight_distribution_strategy: str = "equal",
) -> dict[str, object]:
    assistant_text = _resolve_explicit_assistant_text(sample) or build_assistant_response_text(sample)
    aligned = align_sample_to_tokens(
        sample=sample,
        tokenizer=tokenizer,
        messages=sample.get("messages") if isinstance(sample.get("messages"), list) else None,
        assistant_text=assistant_text,
        weight_distribution_strategy=weight_distribution_strategy,
    )
    return {
        **dict(sample),
        "alignment_metadata": aligned["alignment_metadata"],
        "token_weight_mask": aligned["token_weight_mask"],
    }


def recalibrate_alignment_dataset(
    samples: list[dict[str, object]],
    tokenizer: object,
    weight_distribution_strategy: str = "equal",
) -> list[dict[str, object]]:
    return [
        recalibrate_alignment_sample(
            sample=sample,
            tokenizer=tokenizer,
            weight_distribution_strategy=weight_distribution_strategy,
        )
