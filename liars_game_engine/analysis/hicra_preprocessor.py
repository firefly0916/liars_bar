...
DEFAULT_PROXY_GUIDANCE_SIGNAL_SCALE = 0.1
DEFAULT_REASONING_MISMATCH_SIGNAL_FLOOR = 0.05
...
def resolve_alignment_token_penalty(
    *,
    ev_gap: float,
    best_action_delta: float,
    reasoning_action_mismatch: bool,
    strategic_tokens: list[dict[str, object]] | object,
    proxy_guidance_signal_scale: float = DEFAULT_PROXY_GUIDANCE_SIGNAL_SCALE,
    reasoning_mismatch_signal_floor: float = DEFAULT_REASONING_MISMATCH_SIGNAL_FLOOR,
) -> float:
    if not isinstance(strategic_tokens, list) or not strategic_tokens:
        return 0.0

    signal_strength = abs(float(ev_gap))
    signal_strength = max(
        signal_strength,
        float(proxy_guidance_signal_scale) * abs(float(best_action_delta)),
    )
    if bool(reasoning_action_mismatch):
        signal_strength = max(signal_strength, float(reasoning_mismatch_signal_floor))
    return -signal_strength if signal_strength > 0.0 else 0.0
...
def build_alignment_sample(
    ...
    token_penalty = resolve_alignment_token_penalty(
        ev_gap=ev_gap,
        best_action_delta=best_action_delta,
        reasoning_action_mismatch=reasoning_action_mismatch,
        strategic_tokens=base_sample.get("strategic_tokens", []),
    )
