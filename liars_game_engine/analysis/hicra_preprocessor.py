...
from liars_game_engine.agents.prompts import build_openai_messages, build_prompt, load_prompt_profile
...
def _validate_messages(messages: object) -> list[dict[str, str]]:
    ...
    for index, message in enumerate(messages):
        ...
        content = _ensure_english_only_text(message.get("content", ""), field_name=f"messages[{index}].content")
...
def build_prompt_messages(
    ...
    profile = load_prompt_profile(prompt_profile)
    if prompt_profile == "alignment_action":
        messages = build_openai_messages(profile=profile, observation=observation)
        base_prompt = str(messages[1]["content"])
    else:
        base_prompt = build_prompt(profile=profile, observation=observation)
        messages = [
            {"role": "system", "content": str(profile.get("system", "") or "")},
    ...
    base_prompt = _ensure_english_only_text(base_prompt, field_name="base_prompt")
    rendered_prompt = _ensure_english_only_text(rendered_prompt, field_name="rendered_prompt")
...
def build_holographic_alignment_sample(
    sample: dict[str, object],
    observation: dict[str, object],
    tokenizer: object,
    prompt_profile: str = "alignment_action",
    weight_distribution_strategy: str = "equal",
) -> dict[str, object]:
    prompt_bundle = build_prompt_messages(
        observation=observation,
        tokenizer=tokenizer,
        prompt_profile=prompt_profile,
    )
    enriched = {
        **dict(sample),
        ...
        "rendered_prompt": prompt_bundle["rendered_prompt"],
        "messages": prompt_bundle["messages"],
    }
    return attach_token_alignment(
        sample=enriched,
        tokenizer=tokenizer,
        messages=prompt_bundle["messages"],
        weight_distribution_strategy=weight_distribution_strategy,
...
def build_savi_alignment_full_dataset(
    *,
    report_path: Path | str,
    ev_gap_csv_path: Path | str,
    log_root: Path | str,
    ...
    record_index = load_task_m_record_index(log_root)
    base_samples = build_savi_alignment_dataset(
        report_path=report_path,
        ev_gap_csv_path=ev_gap_csv_path,
        log_root=log_root,
    )
    enriched_samples: list[dict[str, object]] = []
    for sample in base_samples:
        game_id = str(sample.get("game_id", "") or "")
        turn = _safe_int(sample.get("turn", 0), default=0)
        player_id = str(sample.get("player_id", "") or "")
        raw_record = record_index.get((game_id, turn, player_id))
        ...
        enriched_samples.append(
            build_holographic_alignment_sample(
                sample=sample,
                observation=observation,
                tokenizer=tokenizer,
                prompt_profile=prompt_profile,
...
def export_savi_alignment_full_dataset(samples: Iterable[dict[str, object]], output_path: Path | str) -> Path:
    path = Path(output_path)
    path.parent.mkdir(parents=True, exist_ok=True)
    serialized = [dict(sample) for sample in samples]
    path.write_text(
