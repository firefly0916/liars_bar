...
- `task_k_backfill_labels` 现支持 `progress.log` 与批处理进度输出：可通过 `--progress-interval-logs` 控制每批处理多少个 JSONL，并在 `output_dir` 的上级目录下持续追加 `progress.log`，适合服务器后台运行后用 `tail -f` 追踪。
